...
module.exports = function(dependencies) {
  ...
  function Endpoint(options) {
    ...
    this._lastSuccessPingedTime = undefined;
  ...
  Endpoint.prototype._setupHTTP2HealthCheck = function healthcheck() {
    ...
    return setInterval(() => {
      if (Date.now() > (this._pingedThreshold+ this._lastSuccessPingedTime)) {
        ...
        //fix memory leak warning due to too many event
        this._connection.emit('wakeup');
      }
    }, this._heartBeatInterval);
  };

  Endpoint.prototype._connect = function connect(callback) {
    this._createSocket((error, socket) => {
      if (error) {
        this._error(error);
  ...
  Endpoint.prototype._createSocket = function createSocket(callback) {
    if (this.options.proxy) {
      tunnel.createTunnelSocket(this.options, (error, socket) => {
        if (error) {
          return callback(error);
        } else {
          let options = Object.assign({}, this.options);
          options.socket = socket;
          return callback(undefined, tls.connect(options));
        }
      });
    } else {
      return callback(undefined, tls.connect(this.options));
    }
  };
  ...
  Endpoint.prototype.close = function close() {
    if (this._acquiredStreamSlots === 0 && this._connection) {
  ...
  Endpoint.prototype.destroy = function destroy() {
    if (this._heartBeatIntervalCheck) {
